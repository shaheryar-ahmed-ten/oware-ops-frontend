...
function VehicleTypeView() {
  ...
  const vehicleTypeDetailsViewModal = (
    <VehicleTypeDetailsView
      selectedVehicleType={selectedVehicleType}
  ...
  const _getVehicleTypes = (page, searchKeyword) => {
    axios.get(getURL("vehicle-types"), { params: { page, search: searchKeyword } }).then((res) => {
  ...
  const getRelations = () => {
    axios.get(getURL("vehicle-types/relations")).then((res) => {
