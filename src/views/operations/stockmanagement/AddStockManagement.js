...
export default function AddStockManagement() {
  ...
  const [selectedInventoryWastages, setSelectedInventoryWastages] = useState(null),
  ...
  const getReasonsType = () => {
    axios
      .get(getURL(`inventory-wastages/wastages-type`))
      .then((res) => {
      ...
      .then((res) => {
        if (res.data) {
          ...
          const modifiedInventories = res.data.data.Inventories.map((inventory) => {
            ...
            inventory.AdjustmentInventory["remainingQuantity"] =
              inventory.availableQuantity + inventory.AdjustmentInventory.adjustmentQuantity;
            inventory.AdjustmentInventory["actualAvailableQuantity"] =
              inventory.availableQuantity + inventory.AdjustmentInventory.adjustmentQuantity; // saving qty before adjustment
  ...
  const updateAdjustmentsTable = () => {
    if (isRequired(customerId) && isRequired(warehouseId) && isRequired(productId) && isRequired(quantity)) {
      ...
      if (
        ...
        setShowMessage({
          message:
            "This product is already added for this company from this warehouse, please choose a different product/company/warehouse.",
        });
      }
      // if no
      else {
  ...
  const verifyEditedAdjustmentQty = () => {
    return new Promise((resolve, reject) => {
      for (const inventory of selectedInventoryWastageInventories) {
        ...
        if (
          !isRequired(inventory.AdjustmentInventory.adjustmentQuantity) &&
          !inventory.AdjustmentInventory["softDelete"]
        )
          return reject(false);
      }
      return resolve(true);
          ...
          <Grid container item xs={12} justifyContent="space-between">
            <Grid item xs={11}>
              <Typography variant="h3" className={classes.heading}>
                {" "}
                {!!selectedInventoryWastages ? "Edit Stock Adjustment" : "Add Stock Adjustment"}{" "}
              </Typography>
            </Grid>
            <Grid item xs={1}>
              ...
              <TextField
                ...
                onChange={(e) =>
                  e.target.value < 0
                    ...
                    : setQuantity(Math.round(availableQuantity))
                }
              ...
              <FormControl margin="normal" fullWidth={true} variant="outlined">
                <Autocomplete
                  ...
                  getOptionLabel={(reasons) =>
                    reasons.name.charAt(0).toUpperCase() + reasons.name.slice(1).toLowerCase() || ""
                  }
                  onChange={(event, newValue) => {
                    if (newValue) {
                ...
                />
                {validation.reasonType && !isRequired(reasonType) ? (
                  <Typography color="error">Reason type is required!</Typography>
                ) : (
                  <Typography color="error" style={{ visibility: "hidden" }}>
                    Dummy
                  </Typography>
                    ...
                    <TableCell>
                      <DeleteIcon
                        ...
                        onClick={() => {
                          ...
                          setAdjustmentsSecondaryArray(
                            adjustmentsSecondaryArray.filter((adjustmentsSecondaryArray, _idx) => _idx != idx)
                          );
                        }}
                      />
                    </TableCell>
          ...
          <Table stickyHeader aria-label="sticky table">
            <TableHead>
              <TableRow>
                ...
                <TableCell style={{ background: "transparent", fontWeight: "bolder", fontSize: "12px", minWidth: 150 }}>
                  COMMENT
                </TableCell>
                <TableCell>ACTION</TableCell>
              </TableRow>
            ...
            <TableBody>
              {selectedInventoryWastageInventories
                .filter((inventory) => !inventory.AdjustmentInventory["softDelete"])
                .map((inventory, idx) => {
                  return (
                    <TableRow hover role="checkbox" key={idx}>
                      <TableCell>{inventory.Company.name || "-"}</TableCell>
                            ...
                            {reasons.map((reason) => {
                              return (
                                <MenuItem value={reason.id}>
                                  {reason.name.charAt(0).toUpperCase() + reason.name.slice(1).toLowerCase()}
                                </MenuItem>
                              );
                            })}
                          </Select>
      ...
      {adjustmentsSecondaryArray.length > 0 || selectedInventoryWastageInventories.length > 0 ? (
        <Grid container className={classes.parentContainer} xs={12} spacing={3}>
          <Grid item xs={3}>
            <FormControl margin="dense" fullWidth={true} variant="outlined">
              <Button
                onClick={!selectedInventoryWastages ? handleSubmit : handleUpdate}
                color="primary"
                variant="contained"
              >
