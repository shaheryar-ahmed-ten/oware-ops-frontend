...
export default function StockManagementView() {
          ...
          <Grid container xs={4} className={classes.backdropGrid} justifyContent="flex-end">
            <Grid item xs={12}>
              ...
              <Button
                ...
                onClick={() => deleteAdjustment(entityToBeDeleted)}
              >
                Confirm
              </Button>
            </Grid>
  ...
  const deleteAdjustment = (adjustmentId) => {
    axios
      .delete(getURL(`inventory-wastages/${adjustmentId}`))
      .then((res) => {
        if (!res.data.success) {
          setFormErrors(
  ...
  const getRelations = () => {
    axios
      .get(getURL(`product-inward/relations`))
      .then((response) => {
  ...
  const getinventoryWastages = useCallback(
    debounce((page, searchKeyword, selectedWarehouse, selectedProduct, selectedCompany, selectedDay) => {
      _getinventoryWastages(page, searchKeyword, selectedWarehouse, selectedProduct, selectedCompany, selectedDay);
    }, 500),
    []
  );

  const addStockMangementButton = (
    <Button key={2} variant="contained" color="primary" size="small" onClick={() => navigate("create")}>
  ...
  const companySelect = (
    <SelectDropdown
      ...
      setSelectedType={setSelectedCompany}
      setPage={setPage}
    />
  );
  const daysSelect = (
