...
export default function AddProductInwardView() {
  ...
  const updateProductsTable = () => {
    if (isRequired(quantity) && isRequired(customerId) && isRequired(productId) && isRequired(warehouseId)) {
            ...
            <Autocomplete
              ...
              defaultValue={
                selectedProductInward
                  ? { name: selectedProductInward.Company.name, id: selectedProductInward.Company.id }
                  : ""
              }
              options={customers}
        ...
        <Grid item sm={6}>
          ...
          >
            <Autocomplete
              ...
              defaultValue={
                selectedProductInward
                  ? { name: selectedProductInward.Warehouse.name, id: selectedProductInward.Warehouse.id }
                  : ""
              }
              options={warehouses}
        ...
        <Grid container alignItems="center" spacing={2} className={classes.subContainer}>
          ...
          <Grid item xs={2}>
            <TextField
              ...
              disabled={viewOnly}
              onChange={(e) =>
                setQuantity(e.target.value < 0 ? e.target.value == 0 : parseInt(Math.round(e.target.value)))
              }
          ...
          <Grid item xs={2} className={classes.parentContainer}>
            <FormControl
              ...
              variant="outlined"
            >
              <Button variant="contained" onClick={updateProductsTable} color="primary" variant="contained">
                Add Product
              </Button>
            </FormControl>
          </Grid>
                ...
                <TableRow hover role="checkbox">
                  ...
                  <TableCell>
                    <DeleteIcon
                      color="error"
                      key="delete"
                      onClick={() => setProductGroups(productGroups.filter((_productGroup, _idx) => _idx != idx))}
                    />
                  </TableCell>
                </TableRow>
              );
