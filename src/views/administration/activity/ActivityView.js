...
import {
  ...
  cancelFlatTemplate,
  stockadjustment,
} from "../../../utils/activitiyLogTemplates";
...
function ActivityView() {
  ...
  const _getAuditLogs = (page, searchKeyword, selectedDay) => {
    axios.get(getURL("audit-logs"), { params: { page, search: searchKeyword, days: selectedDay } }).then((res) => {
  ...
  const getAuditLogs = useCallback(
    debounce((page, searchKeyword, selectedDay) => {
            ...
            {activityLogs.map((activityLog, idx) => {
              ...
              switch (activityLog.activityType) {
                case "ADD":
                  ...
                  return (
                    <Grid item container xs={12} justifyContent="center" key={idx}>
                      {allowDivider &&
                      (dividerDateFormat(activityLog.updatedAt) !== dividerDateFormat(dateTrack) || idx === 0) ? (
                ...
                case "CANCEL":
                  // TODO: separate template for stock adjustment
                  // const editPayloadData = activityLog.ActivitySourceType.name.toLowerCase() === stockadjustment ?  editFlatTemplate(activityLog) : editFlatTemplate(activityLog)
                  const cancelPayloadData = cancelFlatTemplate(activityLog);
                  return (
                    <Grid item container xs={12} justifyContent="center" key={idx}>
                      {allowDivider &&
                      (dividerDateFormat(activityLog.updatedAt) !== dividerDateFormat(dateTrack) || idx === 0) ? (
                      ...
                      <ActivityDetailsBox activityType={activityLog.activityType} payloadData={cancelPayloadData} />
                    </Grid>
                  );
                case "DELETE":
