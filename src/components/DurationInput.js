...
const DurationTrack = ({ numbers, qualifier, border, onChange,value }) => {
    ...
    const renderNumberJSX = () => {
        return numbers.map((number, i) => (
            value ? <StyledNumber key={qualifier + value}>{value}</StyledNumber>
...
const DurationInput = ({ label, setDuration, duration, setETA, selectedRide }) => {
    ...
    useEffect(() => {
        if (!selectedRide) {
        // console.log("console 1 rerender")
        // if(selectedRide){secondsToDhms(selectedRide)}
        const value = days * 8640 + hours * 3600 + minutes * 60;
        setDuration(value);
        console.log("add",duration, setETA)
        setETA(duration)
        }
    }, [days,minutes,hours]);
    ...
    useEffect(()=>{
        // if (selectedRide) {
            ...
            console.log("dasysysysys",days,hours,minutes)
            const value = days * 8640 + hours * 3600 + minutes * 60;
            console.log("vaue edit",value)
            setDuration(value);
            console.log("duration edit",duration)
            setETA(duration)
            console.log("eta edit",setETA)
        // }
    ...
    return (
        <>
        {console.log("return render",days,hours,minutes)}
            <div>{label}</div>
            <StyledContainer>
                <DurationTrack
